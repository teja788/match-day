...
export async function fetchFootballMatches(): Promise<Match[]> {
  ...
  try {
    ...
    const res = await fetch(
      `${FOOTBALL_API_BASE}/fixtures?date=${today}&timezone=Asia/Kolkata`,
      {
        headers: {
    ...
    console.error('[FootballAdapter] Fetch error:', err);
    return getMockFootballMatches();
  }
}

export async function fetchFootballMatch(fixtureId: string): Promise<Match | null> {
  const RAPIDAPI_KEY = process.env.RAPIDAPI_KEY;
  ...
  try {
    const res = await fetch(
      `${FOOTBALL_API_BASE}/fixtures?id=${fixtureId}`,
      {
        headers: {
          'x-rapidapi-key': RAPIDAPI_KEY,
          'x-rapidapi-host': 'api-football-v1.p.rapidapi.com',
    ...
    console.error('[FootballAdapter] Match fetch error:', err);
    return null;
  }
}

function normalizeFootballMatch(raw: FootballFixture): Match {
...
function abbreviateFootball(name?: string): string {
  if (!name) return '???';
...
function getMockFootballMatches(): Match[] {
  return [
    {
      id: 'football-mock-1',
      sport: 'football',
